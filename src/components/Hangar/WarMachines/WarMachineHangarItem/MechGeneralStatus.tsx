...
export const MechGeneralStatus = ({ mechQueuePosition }: { mechQueuePosition: number }) => {
    ...
    return (
        <Box sx={{ px: "1.6rem", py: ".6rem", backgroundColor: `${primaryColor}10`, border: `${primaryColor} 1.5px dashed` }}>
            ...
            {mechQueuePosition === 0 && (
                <Typography variant="body2" sx={{ color: "green", textAlign: "center", fontFamily: fonts.nostromoBlack }}>
                    IN BATTLE
                </Typography>
            )}
            {mechQueuePosition > 0 && (
                <Typography variant="body2" sx={{ color: "orange", textAlign: "center", fontFamily: fonts.nostromoBlack }}>
