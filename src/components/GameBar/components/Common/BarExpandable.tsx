...
import { ActiveBars, useBar } from "../../containers"
...
export const BarExpandable = ({ barName, iconComponent, children, noDivider }: BarExpandableProps) => {
	...
	return (
		<>
			...
			>
				<Stack justifyContent="center" sx={{ position: "relative", height: "100%", width: "100%", opacity: isActive ? 1 : 0, transition: "all .3s" }}>
					{children}
					<IconButton
						size="small"
						disabled={!isActive}
				...
				>
					<IconButton disabled={isActive} sx={{ mr: 0.5, color: "#FFFFFF" }} onClick={() => toggleActiveBar(barName, true)}>
						{iconComponent}
					</IconButton>

					{!noDivider && (
						<Divider orientation="vertical" flexItem sx={{ height: 23, my: "auto !important", pl: 1, borderColor: colors.darkGrey, borderRightWidth: 1.6 }} />
					)}
				</Stack>
			</Box>
		</>
