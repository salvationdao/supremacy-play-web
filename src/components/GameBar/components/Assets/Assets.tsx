...
const DrawerContent = ({ passportWeb }: { passportWeb: string }) => {
	...
	const queuedWarMachine = useCallback((hash: string) => queuingList.find((q) => q.warMachineMetadata.hash === hash), [queuingList])
	...
	return (
		<Stack key={refresh} sx={{ flex: 1 }}>
			<Stack
				...
				spacing={1.2}
				alignItems="center"
				sx={{ position: "relative", pl: 2.5, pr: 6, height: GAME_BAR_HEIGHT, background: `${colors.assetsBanner}65`, boxShadow: 1.5 }}
			>
				<SvgAssets size="23px" fill={colors.text} />
				<Typography variant="caption" sx={{ flex: 1, color: colors.text, fontFamily: "Nostromo Regular Black" }}>
					ASSETS
				</Typography>
					...
					<Stack>
						{supremacyAssetHashes && supremacyAssetHashes.length > 0 ? (
							supremacyAssetHashes.map((id, index) => (
								<AssetItem key={id} index={index} hash={id} queueDetail={queuedWarMachine(id)} passportWeb={passportWeb} />
							))
						) : (
