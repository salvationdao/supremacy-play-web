...
import { SvgAssets, SvgChat, SvgRobot } from "../assets"
import { GAME_BAR_HEIGHT, LIVE_CHAT_DRAWER_BUTTON_WIDTH } from "../constants"
import { useAuth, useDrawer } from "../containers"
...
export const DrawerButtons = ({ isFixed = true }: { isFixed?: boolean }) => {
	const { user } = useAuth()
	const { isLiveChatOpen, toggleIsLiveChatOpen, isAssetOpen, toggleIsAssetOpen, isMechQueueOpen, toggleIsMechQueueOpen } = useDrawer()
			...
			>
				{user && (
					<>
						<SideButton
							isEnabled={true}
							isOpen={isMechQueueOpen}
							toggleIsOpen={toggleIsMechQueueOpen}
							text="BATTLE QUEUE"
							Svg={<SvgRobot size="12px" sx={{ pt: 0 }} />}
						/>

						<SideButton isEnabled={true} isOpen={isAssetOpen} toggleIsOpen={toggleIsAssetOpen} text="ASSETS" Svg={<SvgAssets size="11px" sx={{ pt: 0.1 }} />} />
					</>
				)}
