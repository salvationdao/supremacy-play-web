...
export const ChatMessage = ({ chat, passportWeb, isSent, isFailed }: { chat: ChatData; passportWeb: string; isSent?: boolean; isFailed?: boolean }) => {
	...
	return (
		<Stack direction="row" spacing={0.5} sx={{ opacity: isSent ? 1 : 0.45 }}>
			...
			<Typography variant="body2" sx={{ color: colors.text, fontFamily: "Share Tech", fontSize: "0.8rem", wordBreak: "break-word", userSelect: "text" }}>
				<span style={{ color: messageColor, fontWeight: 700 }}>{fromUsername}</span>: {cleanedMessage}{" "}
				<span style={{ display: "inline-block", color: "grey", opacity: 0.5, marginLeft: "3px", fontSize: ".7rem" }}>{dateFormatter(sentAt)}</span>
			</Typography>
		</Stack>
	)
}
