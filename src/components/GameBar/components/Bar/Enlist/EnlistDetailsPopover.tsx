...
import { FactionGeneralData, FactionStat } from "../../../types"
...
const PopoverContent = ({ factionData, gameBarBaseProps }: { factionData: FactionGeneralData; gameBarBaseProps: GameBarBaseProps }) => {
	const factionStat = useSubscription<FactionStat>(HubKey.SubscribeFactionStat, { factionID: factionData.id }).payload
	...
	const enlistFaction = useCallback(async () => {
		...
		try {
			await send<any, EnlistFactionRequest>(HubKey.EnlistFaction, { factionID: factionData.id })
		} catch (e) {
		...
		<Stack direction="row">
			...
			<Stack sx={{ px: 3, py: 2.8, width: 460 }}>
				...
				<Stack
					...
					sx={{ mt: "auto", pt: 1.6, "& > .Mui-disabled": { color: `${colors.darkerGrey} !important` } }}
				>
					{factionStat && (
						<>
...
export const EnlistDetailsPopover = ({ popoverRef, popoverOpen, togglePopoverOpen, factionID, factionData, gameBarBaseProps }: EnlistDetailsProps) => {
	const {
		theme: { primary },
	} = factionData
	...
	return (
		<Popover
			...
			anchorOrigin={{
				vertical: barPosition === "top" ? "bottom" : "top",
				horizontal: "center",
			}}
			transformOrigin={{
				vertical: barPosition,
				horizontal: "center",
			}}
		...
		>
			<Box sx={{ filter: "drop-shadow(0 3px 3px #00000050)" }}>
				...
				>
					<Stack direction="row" sx={{ backgroundColor: colors.darkGreyBlue }}>
						{factionData ? (
							<PopoverContent factionData={factionData} gameBarBaseProps={gameBarBaseProps} />
						) : (
							<Box sx={{ px: 3, py: 1.5 }}>
