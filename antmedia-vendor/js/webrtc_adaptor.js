...
export class WebRTCAdaptor {
    constructor(initialValues) {
        ...
        for (var key in initialValues) {
            ...
            var hasBarProperty = {}.hasOwnProperty.call(initialValues, key)
            if (hasBarProperty) {
	...
	*/
    ...
    switchVideoCameraCapture(streamId, deviceId, onEndedCallback) {
        ...
        navigator.mediaDevices.enumerateDevices().then((devices) => {
            ...
            console.debug(
                "Given deviceId = " + deviceId + " - Media constraints video property = " + this.mediaConstraints.video,
            )
            this.setVideoCameraSource(streamId, this.mediaConstraints, null, true, deviceId)
        })
    }
        ...
        if (this.remotePeerConnection[streamId] == null) {
            ...
            if (this.dataChannelEnabled) {
                ...
                if (dataChannelMode == "publish") {
                    ...
                    if (this.remotePeerConnection[streamId].createDataChannel) {
                        var dataChannelPeer = this.remotePeerConnection[streamId].createDataChannel(
                            streamId,
                            dataChannelOptions,
                        )
    ...
    checkWebSocketConnection() {
        if (
            this.webSocketAdaptor == null ||
            (this.webSocketAdaptor.isConnected() == false && this.webSocketAdaptor.isConnecting() == false)
        ) {
